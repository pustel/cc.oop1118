...
public class Molecule {
	...
	public static void main(String[] args) {
		...
		System.out.print("Wieviele Atome soll das Molekül haben? ");
		int n = kbd.nextInt();
//		kbd.close();
...
//		System.out.println("Welcher Periode ehört es an?");
...
//		atom[i].setPeriod(period);
		...
		int i=0;
		private ArrayList <Integer,String> counter = new ArrayList<Integer,String>();
		counter.add(atom[i].getProtonNumber(): atom[i].getSymbol());
		i++;
		while (i <= n) {
			for (int j = 0; j <=i;j++) {
				int t;
